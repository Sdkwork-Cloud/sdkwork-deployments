...
function DomainZoneList({ locale }: { locale: DeploymentsLocale }) {
  ...
  return <section className="resource-page domain-page">
    <header className="page-header">
      <div><span className="eyebrow">DNS</span><h1>{t("domainsTitle")}</h1><p>{t("domainsSubtitle")}</p></div>
      <button className="icon-button" type="button" disabled={busy} title={t("refresh")} onClick={reload}><RefreshCw size={18} /></button>
    </header>
...
function DomainHostnameList({ locale }: { locale: DeploymentsLocale }) {
  ...
  return <section className="resource-page domain-page">
    ...
    <header className="page-header domain-detail-header">
      <div><span className="eyebrow">DNS ZONE</span><h1>{zone?.apexHostname ?? "-"}</h1><p>{t("subdomainsSubtitle", { domain: zone?.apexHostname ?? "-" })}</p></div>
      <div className="header-actions"><button className="icon-button" type="button" disabled={busy} title={t("refresh")} onClick={reload}><RefreshCw size={18} /></button><button className="command-button" type="button" onClick={() => setCreateOpen(true)}><Plus size={16} />{t("addHostname")}</button></div>
    </header>
...
function hasRelations(...values: string[]): boolean {
