...
users:
  ...
  - public readers
goals:
  - publish eligible Drive folders and Wiki publications without a release for each file change
...
1. `sdkwork-deployments` shall be the only writable authority for Sites, host/path Bindings,
   ...
   observations.
2. Drive shall expose only a selected folder in an active `website` Space as `DRIVE_DIRECTORY`.
3. Knowledgebase shall expose only an active `wiki` publication rooted at `sources/raw` as
   `KNOWLEDGEBASE_WIKI`, with per-file publication and visibility enforcement.
...
- Contract tests prove eligibility rules for ordinary Drive, Website Space, ordinary Knowledgebase,
  and active Wiki publication.
...
- Module requirements: Drive `REQ-2026-0004`, Knowledgebase `REQ-2026-0722`, Web Server
  `REQ-2026-0060`
