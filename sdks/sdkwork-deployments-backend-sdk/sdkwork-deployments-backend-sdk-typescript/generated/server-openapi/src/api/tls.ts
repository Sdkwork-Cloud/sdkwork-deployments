...
/** List ACME accounts of the tenant */
  async list(params?: TlsTlsAccountsListParams, requestOptions?: ApiRequestOptions): Promise<{ items: AcmeAccountResponse[]; pageInfo: PageInfo; }> {
    const query = buildQueryString([
      { name: 'page', value: params?.page, style: 'form', explode: true, allowReserved: false },
      { name: 'page_size', value: params?.pageSize, style: 'form', explode: true, allowReserved: false },
    ]);
...
export function createTlsApi(client: HttpClient): TlsApi {
  return new TlsApi(client);
}
