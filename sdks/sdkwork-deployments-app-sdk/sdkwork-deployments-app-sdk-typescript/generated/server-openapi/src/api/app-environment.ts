...
import type { AppEnvironmentResponse, CreateAppEnvironmentRequest, EnvironmentPromotionResponse, PageInfo, PromoteEnvironmentRequest, UpdateAppEnvironmentRequest } from '../types';
...
export interface AppEnvironmentListPromotionsParams {
  page?: number;
  pageSize?: number;
}
...
/** List the immutable promotion history of an environment */
  async listPromotions(appId: string, environmentId: string, params?: AppEnvironmentListPromotionsParams, requestOptions?: ApiRequestOptions): Promise<{ items: EnvironmentPromotionResponse[]; pageInfo: PageInfo; }> {
    const query = buildQueryString([
      { name: 'page', value: params?.page, style: 'form', explode: true, allowReserved: false },
      { name: 'page_size', value: params?.pageSize, style: 'form', explode: true, allowReserved: false },
    ]);
