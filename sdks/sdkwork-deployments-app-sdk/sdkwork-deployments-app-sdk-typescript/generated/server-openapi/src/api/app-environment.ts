...
import type { AppEnvironmentResponse, CreateAppEnvironmentRequest, PageInfo, UpdateAppEnvironmentRequest } from '../types';


export interface AppEnvironmentListParams {
  page?: number;
  pageSize?: number;
}

export interface AppEnvironmentCreateParams {
  idempotencyKey: string;
}

export interface AppEnvironmentUpdateParams {
  idempotencyKey: string;
}

export class AppEnvironmentApi {
...
/** List the environments of an app */
  async list(appId: string, params?: AppEnvironmentListParams, requestOptions?: ApiRequestOptions): Promise<{ items: AppEnvironmentResponse[]; pageInfo: PageInfo; }> {
...
/** Create an environment in the promotion chain */
  async create(appId: string, body: CreateAppEnvironmentRequest, params: AppEnvironmentCreateParams, requestOptions?: ApiRequestOptions): Promise<AppEnvironmentResponse> {
...
/** Update an environment (name, approval requirement, status) */
  async update(appId: string, environmentId: string, body: UpdateAppEnvironmentRequest, params: AppEnvironmentUpdateParams, requestOptions?: ApiRequestOptions): Promise<AppEnvironmentResponse> {
    const requestHeaders = buildRequestHeaders(
      {
        'Idempotency-Key': { value: params.idempotencyKey, style: 'simple', explode: false },
      },
...
export function createAppEnvironmentApi(client: HttpClient): AppEnvironmentApi {
  return new AppEnvironmentApi(client);
}
